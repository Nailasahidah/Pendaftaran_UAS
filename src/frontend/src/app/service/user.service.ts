...
import {AccountType, CustomerState, CustomHttpResponse, Page, Profile, UserState} from "../interface/appstates";
...
export class UserService {
  ...
  profiles$ = (page: number = 0) => <Observable<CustomHttpResponse<Page<User> & User & Stats>>>
  ...
  prof$ = (profileId: number) => <Observable<CustomHttpResponse<UserState>>>
    this.http.get<CustomHttpResponse<UserState>>
    (`${this.server}/user/get/${profileId}`)
      .pipe(
        tap(console.log),
        catchError(this.handleError)
      );
